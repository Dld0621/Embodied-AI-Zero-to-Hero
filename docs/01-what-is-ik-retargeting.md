...
**优点**：相对当前状态描述修正量，适合闭环控制；仍必须声明增量在哪个参考系中表达
**缺点**：累积误差；需要初始姿态
...
- **比例缩放**：在明确的任务坐标系中按长度比缩放位置/位移目标；角度并不普遍按长度比缩放
...
仅在上面约定的掌局部坐标系及镜像平面下，才能用 Y 取反实现这一点坐标镜像。镜像变换的行列式为 −1，不是 SO(3) 旋转；一般左右手映射必须同时核对关节轴、手性、点序和动作语义，不能对任意相机世界坐标直接套用。
