...
> **内容状态：已读，仍有已确认待修项（2026-09-05）。** 本页合成输入演示可运行，不证明原 DexMV 复现、真实输入精度或 100 Hz 端到端控制（补审 F07）。正文相关结论仍待逐段修正；请先看 [已修示例说明](../examples/dexmv_style_retargeting/README.md)，不接真实机器人。 具体位置与原始来源见 [补充独立审查](reviews/remaining-source-review.md)。
