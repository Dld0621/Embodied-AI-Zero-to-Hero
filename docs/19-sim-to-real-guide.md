...
> **逐点图解 / Concept close-ups：**[系统辨识与 Sim-to-Real](knowledge-atlas/deploy-sim-to-real/index.md)。每个小点配原理、算例、图、自测；这是中文细解，保留英文术语。
...
> [!CAUTION]
> **已知问题／不可直接用于真机：**本页仍包含未经目标 MuJoCo 版本编译、传感器核对或真实机器人验证的参数与代码片段；域随机化、MJCF、VLM、触觉和故障处置还存在已登记的开放问题。它们只能作为审查中的教学草案，不能复制为真机控制、安全层或放行依据。开始任何硬件试验前，必须以目标机器人的数据手册、实测状态、软硬限位、独立急停和分阶段验证为准。详见独立审查 [F09–F10](reviews/remaining-source-review.md)。
...
│
├── 物体异常滑动
│   ├── 用斜坡/拉力试验校准滑动摩擦，不凭现象单向调大
│   ├── 使用 elliptic friction cone（cone="elliptic"）
│   └── 核对实测质量、接触几何、法向力、速度和加速度；不得通过增加物体质量或盲目抬高接触刚度来掩盖滑动，无法确认时受控停止
│
└── 仿真发散 (NaN)
    ├── 减小 timestep
    ├── 检查质量/惯量是否为正
...
| 失败模式 | 真实表现 | 根因诊断 | 解决方案 |
|---------|---------|---------|---------|
| **物体滑落** | 抓取后物体掉落 | 仿真摩擦过高 / 接触法向力不足 / 运动加速度过大 | 受控停止并保护物体；校准摩擦和法向力，降低运动速度/加速度，在限力与限位内逐步复测；不得盲目增加力控增益 |
| **抖动/振荡** | 手臂/手指高频振动 | 力控增益过高 / 延迟未补偿 / 接触刚度失配 | 降低增益，实施延迟补偿，调整 solref |
| **到达错误位置** | 无法对准物体 | 视觉域差距 / 相机标定误差 / 手眼标定误差 | 视觉域随机化，重新标定相机，检查 extrinsic |
| **过度抓取** | 物体变形/压碎 | 力/位置目标或控制增益过高、允许阈值过大、触觉反馈失效 | 立即受控停止或释放到安全状态；降低并验证力/位置目标、控制增益、软硬限位与触觉阈值，再从保守条件逐步复测 |
