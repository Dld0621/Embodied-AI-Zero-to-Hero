...
本项目有一个**教科书级**的过拟合案例。SmolVLA（450M）在 PushCube 上微调（见 [`docs/28-smolvla-gpu-finetuning-runbook.md`](../28-smolvla-gpu-finetuning-runbook.md)）：
...
> **判断口诀**：训练 loss ↓↓↓ 但闭环成功率不动 → BC 过拟合。这不是"模型不够大"或"loss 不够低"，而是数据/方法问题：50 条专家轨迹太少、行为太单一，模型只能记忆而非泛化。修复方向：更多样化数据（上百条以上）、加正则、或换 RL 在闭环中继续优化。本项目明确把"扩数据 + 更大模型"列为下一步（见 [`README.md`](../../README.md) Contributing）。
