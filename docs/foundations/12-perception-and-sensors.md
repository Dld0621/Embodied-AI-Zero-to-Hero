...
<div class="dof-principle" role="group" aria-label="针孔相机投影原理图">
  <p class="dof-principle__caption"><strong>原理图 · Pinhole projection.</strong> 外参先把世界点带入相机坐标；一条直线光路穿过针孔，在后方物理感光面形成倒像。右下公式采用计算机视觉常用的前方虚拟像平面约定；不要把两种平面直接混用。</p>
  <div class="dof-principle__canvas">
    <svg viewBox="0 0 920 330" role="img" aria-labelledby="camera-figure-title camera-figure-desc">
      ...
      <desc id="camera-figure-desc">One straight ray from a scene point passes through the pinhole to a single inverted image on the physical sensor plane at negative focal distance. The formula uses the separate positive-focal-length virtual image-plane convention.</desc>
      <defs>
      ...
      <text class="dof-diagram-title" x="325" y="38">sensor plane (−f)</text>
      ...
      <path class="dof-diagram-line" d="M730 80 L500 162 L392 200.504"/>
      <circle class="dof-diagram-fill-violet" cx="392" cy="200.504" r="8"/>
      <text class="dof-diagram-label" x="294" y="226">inverted image</text>
      ...
      <text class="dof-diagram-note" x="629" y="267">virtual-plane convention (+f)</text>
    </svg>
  </div>
</div>

**读图约定**：若像面放在针孔后方的 `Z=−f`，且横向坐标轴方向与相机一致，相似三角形给出 `x_image=−f*X/Z`（倒像）。计算机视觉通常用前方 `Z=+f` 的虚拟像平面，并映射到约定的像素轴，因此写成 `u=f_x*X/Z+c_x`、`v=f_y*Y/Z+c_y`。两种画法描述同一束光，但符号和坐标约定不同。外参在此始终将世界点转换到相机坐标，见 [OpenCV 针孔相机模型](https://docs.opencv.org/4.x/d9/d0c/group__calib3d.html)。
