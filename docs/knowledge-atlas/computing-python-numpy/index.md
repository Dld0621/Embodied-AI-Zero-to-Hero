...
[全部知识点](../index.md) · [返回原课程](../../foundations/01-python-for-robotics.md)
...
1. view 持有同一数据缓冲区的另一种索引方式，不一定分配独立数据。
2. 基础切片通常产生 view；经 view 写入会改变原数组的对应位置。
3. 需要保留原始观测时显式 copy，并通过内存共享检查或回归测试确认边界。

### 手算或追踪一个例子

1. 教学输入 a=[1,2,3]；令 v=a[:2]，另令 c=a[:2].copy()。
2. 执行 v[0]=9，会使 a=[9,2,3]；c 仍为 [1,2]。
3. 再执行 c[1]=8，只改变 c=[1,8]，a 保持 [9,2,3]。
